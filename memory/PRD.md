...
- **Phase 1C complete (Feb 23)** — Attendance deepening. Shift system (5 seeded: General 9-6 default, Morning 6-3, Afternoon 2-11, Night 10pm-7am overnight, Flexible 8-20) with weekly-off config, grace minutes, half-day threshold, min-full-day hours, color coding. Shift assignments per employee with date-range windows; active shift auto-resolved at check-in. Work sites with lat/lng + radius (haversine-based geo-fencing); WFH bypasses fence, WFO/Field checks compare GPS coords to assigned sites. Check-in stamps shift + late-mark + late_minutes from shift start + grace. Check-out computes hours + auto half-day detection. Regularization requests (missed_punch, wrong_location, wrong_shift, forgot_checkout) flow through approval engine. Overtime requests (0-12h, 1.0/1.5/2.0x rate) flow through approvals. Weekly timesheets with project/task/hours entries, draft→submitted workflow via approvals. Monthly attendance register (MIS) endpoint generates per-employee-per-day codes (P/P*/A/L/H/WO/HD) and summary; sticky-column table UI. Approval decide handler now dispatches to regularization/overtime/timesheet collections. Frontend: /app/attendance with 6 tabs (Today with GPS geolocation fetch + shift sidebar, My log, Regularization, Overtime, Timesheet weekly editor, Register MIS) and /app/attendance-admin with 3 tabs (Shifts CRUD with weekly-off day pickers + is_overnight + is_default, Shift assignments CRUD, Work sites CRUD). Cross-tenant verified. 17/17 pytest cases passing.
- **Phase 1B complete (Feb 23)** — Leave deepening, India-first. 9 configurable leave types (CL/SL/EL/COMP/ML/PL/BL/MRL/LOP) with policies, per-employee balance ledger, 22 India 2026 holidays, team calendar, leave admin UI.
