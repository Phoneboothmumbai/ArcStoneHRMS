...
- **Phase 1B complete (Feb 23)** — Leave deepening, India-first. 9 configurable leave types seeded (Casual 12d, Sick 12d w/doc-after-3d, Earned 21d monthly-accrual + encashable cap 30, Comp-off, Maternity 182d female-only, Paternity 15d male-only, Bereavement 5d, Marriage 5d with doc + 30d notice, LOP allow-negative-balance). Per-employee-per-type-per-year balance ledger with computed `available = allotted + carried_forward + adjustments − used − pending`. Balance auto-created on first read. Holiday calendar: 22 India 2026 holidays seeded (mandatory + optional), filterable per-year per-branch. Leave apply v2 uses leave_type_id, supports half-day start/end, enforces notice_days, max_consecutive_days, min_service_months, gender eligibility, requires_document hints, allow_negative_balance for LOP, and correctly counts days excluding Sundays + mandatory holidays. Balance reserves pending on apply; transitions pending→used on approve; releases on reject/cancel (full sync with approval engine). HR admin UI at /app/leave-admin with two tabs — Leave types CRUD (color, accrual cadence, carry-forward rules, encashment cap, gender/service eligibility) + Holiday calendar CRUD (mandatory/optional/restricted). Employee Leave page has 3 tabs — My leave (balance cards + all-types table + apply dialog with live balance preview + cancel button), Team calendar (60-day look-ahead of company-wide leaves with color dots), Holidays (month-grouped list). Cross-tenant isolation verified. 18/18 new pytest cases passing.
